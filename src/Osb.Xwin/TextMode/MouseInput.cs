...
/// <summary>Um clique de mouse, em coordenadas de CARACTERE do terminal (coluna/linha, base 1).</summary>
public readonly record struct MouseClick(int Column, int Row, bool IsPress);

/// <summary>Um evento de entrada: ou uma tecla normal, ou um clique de mouse.</summary>
...
{
    /// <summary>Liga o relato de cliques do mouse (modo 1000 = clique, 1006 = coordenadas estendidas/SGR).</summary>
    public static void Enable() => Console.Write("\u001b[?1000h\u001b[?1006h");
    ...
    public static void Disable() => Console.Write("\u001b[?1000l\u001b[?1006l");

    /// <summary>Lê o próximo evento de entrada, bloqueando até haver uma tecla ou clique.</summary>
    public static InputEvent Read()
    {
        ...
        if (terminator != '\0' && parts.Length == 3
            && int.TryParse(parts[1], out var cx) && int.TryParse(parts[2], out var cy))
        {
            // Ajusta coordenadas de 1-based (protocolo SGR) para 0-based usado pelo Console.
            return InputEvent.FromMouse(new MouseClick(cx - 1, cy - 1, terminator == 'M'));
        }
